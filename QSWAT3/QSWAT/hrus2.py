...
# This is a version of hrus.py using the raster.py code for reading and writing raster data in generateBasins.
# But it proved to be slower in practice, so is not used.
...
class HRUs(QObject):
    ...
    def readFiles(self) -> bool:
        ...
        if self._dlg.readFromPrevious.isChecked():
            ...
            time2 = time.process_time()
            self.progress('Generating basins took {0} seconds'.format(int(time2 - time1)))
    ...
    def generateBasins(self, progressBar: QProgressBar, progressLabel: QLabel, root: QgsLayerTree) -> bool:
        ...
        while not completed:
            try:
                ...
                completed = True
                elevationRaster = Raster(self._gv.demFile, self._gv, canWrite=False, isInt=False)
                res = elevationRaster.open(chunkCount)
                if not res:
                    return False
                elevationDs = elevationRaster.ds
                if not self._gv.useGridModel:
                    basinRaster = Raster(self._gv.basinFile, self._gv, canWrite=False, isInt=True)
                    res = basinRaster.open(chunkCount)
                    if not res:
                        return False
                    basinDs = basinRaster.ds
                    if not basinDs:
                        QSWATUtils.error('Cannot open watershed grid {0}'.format(self._gv.basinFile), self._gv.isBatch)
                        return False
                    basinNumberRows = basinRaster.numRows
                    basinNumberCols = basinRaster.numCols
                    fivePercent = int(basinNumberRows / 20)
                    basinTransform = basinDs.GetGeoTransform()
                    basinNoData = basinRaster.noData
                if not self._gv.existingWshed and not self._gv.useGridModel:
                    distRaster = Raster(self._gv.distFile, self._gv, canWrite=False, isInt=False)
                    res = distRaster.open(chunkCount)
                    if not res:
                        return False
                    distDs = distRaster.ds
                cropRaster = Raster(self._gv.landuseFile, self._gv, canWrite=False, isInt=True)
                res = cropRaster.open(chunkCount)
                if not res:
                    return False
                cropDs = cropRaster.ds
                soilRaster = Raster(self._gv.soilFile, self._gv, canWrite=False, isInt=True)
                res = soilRaster.open(chunkCount)
                if not res:
                    return False
                soilDs = soilRaster.ds
                slopeRaster = Raster(self._gv.slopeFile, self._gv, canWrite=False, isInt=False)
                res = slopeRaster.open(chunkCount)
                if not res:
                    return False
                slopeDs = slopeRaster.ds
                

                elevationBand = elevationRaster.band
                
                elevationNoData = elevationRaster.noData
                if self._gv.existingWshed or self._gv.useGridModel:
                    distNoData = elevationNoData
                else:
                    distNoData = distRaster.noData
                cropNoData = cropRaster.noData
                if cropNoData is None:
                    cropNoData = self.defaultNoData
                soilNoData = soilRaster.noData
                if soilNoData is None:
                    soilNoData = self.defaultNoData
                if self._gv.isHUC:
                    self._gv.db.SSURGOUndefined = soilNoData
                slopeNoData = slopeRaster.noData
                if not self._gv.useGridModel:
                ...
                if not self._gv.existingWshed and not self._gv.useGridModel:
                    distNumberRows = distRaster.numRows
                    distNumberCols = distRaster.numCols
                cropNumberRows = cropRaster.numRows
                cropNumberCols = cropRaster.numCols
                soilNumberRows = soilRaster.numRows
                soilNumberCols = soilRaster.numCols
                slopeNumberRows = slopeRaster.numRows
                slopeNumberCols = slopeRaster.numCols
                elevationNumberRows = elevationRaster.numRows
                elevationNumberCols = elevationRaster.numCols
                ...
                if not self._gv.useGridModel and len(self._gv.db.slopeLimits) > 0:
                    ...
                    slopeBandsRaster = Raster(self._gv.slopeBandsFile, self._gv, canWrite=True, isInt=True)
                    slopeBandsNoData = -1
                    res = slopeBandsRaster.open(chunkCount, numRows=slopeNumberRows, numCols=slopeNumberCols, 
                                                transform=slopeTransform, projection=proj, noData=slopeBandsNoData)
                    if not res:
                        return False
                    QSWATUtils.copyPrj(self._gv.slopeFile, self._gv.slopeBandsFile)
                    
                # prepare HRUs raster
                if not self._gv.useGridModel and hrusRasterWanted:
                    ...
                    if not ok:
                        pass #  no great harm with raster
                    hrusRaster = Raster(hrusRasterFile, self._gv, canWrite=True, isInt=True)
                    hrusRasterNoData = -1
                    res = hrusRaster.open(chunkCount, numRows=basinNumberRows, numCols=basinNumberCols, 
                                          transform=basinTransform, projection=proj, noData=hrusRasterNoData)
                    if not res:
                        return False
...
#                 distData = numpy.empty([distReadRows, distNumberCols], dtype=float)  # type: ignore
        if self.fullHRUsWanted or hrusRasterWanted:
            ...
            if self._gv.useGridModel:
                ...
                # numpy.core.full introduced in version 1.8
...
#                             slopeData = slopeBand.ReadAsArray(0, slopeTopRow, slopeNumberCols, slopeActReadRows)
                ...
                for row in rowRange:
                    ...
                    for col in colRange:
                        elevation = elevationRaster.read(row - elevationTopRow, col) * self._gv.verticalFactor
                        ...
                        if 0 <= cropRow - cropTopRow < cropActReadRows:
                            ...
                            if 0 <= cropCol < cropNumberCols:
                                crop = cropRaster.read(cropRow - cropTopRow, cropCol)
                        ...
                        if 0 <= soilRow - soilTopRow < soilActReadRows:
                            ...
                            if 0 <= soilCol < soilNumberCols:
                                soil = soilRaster.read(soilRow - soilTopRow, soilCol)
                        ...
                        if 0 <= slopeRow - slopeTopRow < slopeActReadRows:
                            ...
                            if 0 <= slopeCol < slopeNumberCols:
                                slopeValue = slopeRaster.read(slopeRow - slopeTopRow, slopeCol)
                            else:
...
#                     elevationData = elevationBand.ReadAsArray(0, elevationRow, elevationNumberCols, 1)
                for col in range(basinNumberCols):
                    basin = basinRaster.read(row, col)
                    if basin != basinNoData and not self._gv.topo.isUpstreamBasin(basin):
                        ...
                        if not self._gv.existingWshed:
                            ...
                            if 0 <= distCol < distNumberCols and 0 <= distRow < distNumberRows:
                                dist = distRaster.read(distRow, distCol)
                            else:
                                dist = distNoData
                        else:
                            dist = distNoData
                        ...
                        if 0 <= cropCol < cropNumberCols and 0 <= cropRow < cropNumberRows:
                            crop = cropRaster.read(cropRow, cropCol)
                            if crop is None or math.isnan(crop):
                                crop = cropNoData
                        else:
                            crop = cropNoData
                        ...
                        if 0 <= soilCol < soilNumberCols and 0 <= soilRow < soilNumberRows:
                            soil = soilRaster.read(soilRow, soilCol)
                            if soil is None or math.isnan(soil):
                                soil = soilNoData
                        else:
                            soil = soilNoData
                        ...
                        if 0 <= slopeCol < slopeNumberCols and 0 <= slopeRow < slopeNumberRows:
                            slopeValue = slopeRaster.read(slopeRow, slopeCol)
                        else:
                            slopeValue = slopeNoData
                        ...
                        if len(self._gv.db.slopeLimits) > 0:
                            slopeBandsRaster.write(slopeRow, slopeCol, slope)
                            if slope > 1:
                                slope = slope
                        ...
                        if 0 <= elevationCol < elevationNumberCols and 0 <= elevationRow < elevationNumberRows:
                            elevation = elevationRaster.read(elevationRow, elevationCol) * self._gv.verticalFactor
                        else:
                        ...
                        if self.fullHRUsWanted or hrusRasterWanted:
                            if crop != cropNoData and soil != soilNoData and slope != slopeNoData:
                                ...
                                if hru > lastHru:
                                    ...
                                    lastHru = hru
                                hruRow[col] = hru
                                hrusRaster.write(row, col, hru)
                            else:
                                hruRow[col] = -1
                                hrusRaster.write(row, col, -1)
...
#                 slopeBandsBand.WriteArray(slopeData, 0, slopeCurrentRow)
                ...
                slopeBandsRaster.close()
            if hrusRasterWanted:
                hrusRaster.close()
        # clear some memory
        elevationRaster.close()
        if not self._gv.existingWshed and not self._gv.useGridModel:
            distRaster.close()
        slopeRaster.close()
        soilRaster.close()
