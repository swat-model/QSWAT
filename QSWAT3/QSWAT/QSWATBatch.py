...
/***************************************************************************
 QSWATBatch
 Run QSWAT in batch mode using a project file to provide inputs and parameters
                              -------------------
        begin                : 2015-10-20
...
def main(projFile):
    ...
    # would like to remove whole Watershed directory, 
    # but inlets/outlets shapefile stored in Watershed\Shapes
    QSWATUtils.removeFiles(QSWATUtils.join(projDir, r'Watershed\Shapes\riv1.shp'))
    ...
    if not dlg.delinButton.isEnabled():
        ...
        return -1
    delin = Delineation(iface, plugin._gv, plugin._demIsProcessed)
    delin.init()
    ...
    plugin._gv.db.clearTable('BASINSDATA1')
    hrus = HRUs(iface, plugin._gv, dlg.reportsBox)
    hrus.init()
    hrus.readFiles()
    ...
    if not os.path.exists(QSWATUtils.join(plugin._gv.textDir, Parameters._HRUSREPORT)):
        ...
        return -1
    if not os.path.exists(QSWATUtils.join(projDir, r'Watershed\Shapes\riv1.shp')):
